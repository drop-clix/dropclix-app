...
- Bundle create flow: Suggestion cards in the discovery modal can be toggled into a bundle with the original discovery. `Link & Create New` requires at least one related discovery and blocks duplicate platforms client-side; `createPipelineItemFromDiscoveryBundle()` re-validates the same rule server-side. The action creates one POSTED `pipeline_items` row with pipe-separated platform IDs, a user-confirmed hero title, earliest available publish date, and the selected platform video IDs, then marks every discovery `linked`.
- Bundle sync behavior: After the pipeline item and discovery links are written, the bundle action calls the existing `ensureIGPostsRow` / `ensureTTPostsRow` / `ensureYTPostsRow` + `syncLinkedVideoNow()` path for each selected platform. Sync failures are returned as partial-success errors and logged, but the explicit pipeline/discovery links stay intact; do not roll them back just because a platform API sync failed.
