...
- **Instagram OAuth**: `/api/auth/instagram/route.ts` uses `INSTAGRAM_APP_ID` (server-side, no NEXT_PUBLIC_ prefix needed). Scopes: `instagram_business_basic,instagram_manage_insights,...`. Callback exchanges code → short-lived token → long-lived token (60-day TTL); stores in `platform_connections` with `subscriber_count = followers_count`. **App in Development Mode** — add accounts as testers in Meta Developer Dashboard before testing.
