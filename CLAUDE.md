...
- **video-polling.ts**: `fetchYTPublicStats(videoId)` uses YouTube Data API v3 — needs `YOUTUBE_API_KEY` env var (public key, no OAuth). Different from the Analytics API used by YT sync. `upsertPolledStats()` preserves `prev_views`+`prev_recorded_at` for growth rate interpolation. `autoDiscoverYTUploads` DELETED (S39) — no fuzzy matching, no auto-discover.
