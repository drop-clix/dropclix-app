...
- **`autoDiscoverYTUploads` known IDs**: checks both `post_analytics.yt_id` AND `pipeline_items.yt_video_id` to avoid re-discovering already-linked videos. Fuzzy match window is ±72h (not ±24h).
...
- **video-polling.ts**: `fetchYTPublicStats(videoId)` uses YouTube Data API v3 — needs `YOUTUBE_API_KEY` env var (public key, no OAuth). Different from the Analytics API used by YT sync. `upsertPolledStats()` preserves `prev_views`+`prev_recorded_at` for growth rate interpolation. Auto-discover reads channel uploads playlist (UU... prefix from UC... channel ID).
- **Cron routes**: `/api/cron/poll-fresh|recent|archive` — Hobby plan → daily only. Pro plan enables `*/2`, `*/10`, `0 */6` schedules. Routes respond to GET with `CRON_SECRET` bearer auth (optional). Can be triggered manually.
