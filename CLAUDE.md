...
- S49 (GLOBAL): PKCE magic link redirect fix. Added `src/app/auth/callback/route.ts` — calls `supabase.auth.exchangeCodeForSession(code)` and redirects to `/`; errors redirect to `/login?error=auth_failed`. Updated `emailRedirectTo` in `handleMagic` to `${window.location.origin}/auth/callback`. Added `shouldCreateUser: false` to `signInWithOtp` to prevent creating Supabase auth accounts for unknown emails. Proxy already passes `/auth/` routes unauthenticated. tsc clean. Root cause of second failure: `getUser()` in `proxy.ts` was firing before the `/auth/` passthrough check, consuming the PKCE code before the route handler could exchange it. Fix: moved `/api/` and `/auth/` passthroughs above `createServerClient` + `getUser()` — those routes never touch auth state.
...
- **Magic link / invite token handling**: Two flows coexist. (1) **PKCE flow** (default for `signInWithOtp`): Supabase emails a link with `?code=...` appended to `emailRedirectTo`. The `emailRedirectTo` is set to `${window.location.origin}/auth/callback`. The callback route (`src/app/auth/callback/route.ts`) calls `supabase.auth.exchangeCodeForSession(code)` and redirects to `/`. `shouldCreateUser: false` prevents unknown-email account creation. (2) **Hash/implicit flow**: If the URL has `#access_token=...&type=magiclink|invite` (invite emails, legacy links), `login/page.tsx` `useEffect` detects it and uses `onAuthStateChange(SIGNED_IN)` (not `getSession()`) because Supabase processes the hash asynchronously. `timerRef` guards a 5-second expiry fallback. Proxy passes all `/auth/` routes unauthenticated, including `/auth/callback`. **Critical proxy ordering**: `/api/` and `/auth/` passthroughs must come BEFORE `createServerClient` + `getUser()` in `proxy.ts` — `getUser()` can consume the PKCE code from the query string before the route handler runs.
