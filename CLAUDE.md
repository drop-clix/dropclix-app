...
- S39 `no_posts_row` fix: `pollPipelineItem` upgraded to 3-strategy posts lookup: (1) exact `post_id` match, (2) pipe-split (for multi-platform pipeline items like `#ig0037 | #tt0007 | #yt0087`), (3) `yt_id` fallback (`posts.yt_id = yt_video_id`). `ensureYTPostsRow(pipelineItemId)` server action added to `edit-actions.ts` — auto-creates stub posts row when YT ID is set in Mark as Posted modal. `MarkAsPostedModal.handleConfirm` calls `ensureYTPostsRow` when `parsedIds.yt` is set. Migration SQL: `supabase/migrations/fix_missing_posts_rows.sql` (run in Supabase SQL Editor — safe to re-run). Script: `scripts/fix-missing-posts-rows.mjs` (requires real Supabase creds in `.env.local`).
...
- **`linkYouTubeVideo` durable path**: always saves `yt_video_id` to `pipeline_items` when `pipelineItemId` arg is provided (3rd arg). This is the reload-safe path. `post_analytics.yt_id` update is best-effort (may be 0 rows if post has no analytics). Returns `{ ytId, note? }` — note shown as info toast, never blocks save.
...
- **`ensureYTPostsRow(pipelineItemId)`**: server action in `edit-actions.ts`. Uses same 3-strategy lookup. If no posts row found, creates stub `#ytNNNN` row with `yt_id` set so strategy 3 works on next poll. Called from `MarkAsPostedModal.handleConfirm` when `parsedIds.yt` is set.
...
- **Cron polling model (S39)**: Source of truth = `pipeline_items.yt_video_id` set by admin via Mark as Posted modal or YT link button. No link = not polled. `getPostableItemsInAgeRange` filters `yt_video_id IS NOT NULL`. Archive tier (`maxAgeDays=null`) includes `posted_at IS NULL` via `.or('posted_at.lte.X,posted_at.is.null')`. `pollPipelineItem(admin, item)` — no postUUID param, resolves UUID internally from `posts` table, skips with `reason='no_posts_row'` if not found.
...
- **analytics_snapshots**: permanent records, never overwrite (`UNIQUE (post_id, window_type)`). `snapshot_jobs` drives the scheduling (T+24hr/3day/7day/EOM). Both tables have RLS. Migration: `session_37_snapshots.sql`.
