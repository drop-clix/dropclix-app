...
export default async function AdminMetaAdsSelectPage({
  searchParams,
}: {
  ...
  if (!pending.ok) {
    redirect(`/admin?meta_ads_error=${encodeURIComponent(pending.error)}`)
  }

  if (pending.row.platform !== 'meta_ads' || pending.row.origin !== 'admin') {
    redirect('/admin?meta_ads_error=unauthorized')
  }

  async function chooseAccount(formData: FormData) {
    'use server'
    formData.set('fallback', '/admin')
  ...
  return (
    <div className="min-h-screen" style={{ background: '#060606', padding: '48px 40px' }}>
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        <div style={{ marginBottom: 34, paddingBottom: 26, borderBottom: '1px solid #141414' }}>
          <p
            className="text-[9px] font-medium tracking-[.26em] uppercase mb-3 flex items-center gap-3"
            style={{ color: '#1778f2' }}
          >
          ...
          <p className="text-[12px] font-light mt-2" style={{ color: '#666', maxWidth: 620 }}>
            Multiple active ad accounts are available. Choose the one this client should use for Meta Ads reporting.
          </p>
        </div>
        ...
        <Link
          href="/admin?meta_ads_error=selection_cancelled"
