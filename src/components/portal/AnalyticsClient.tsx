...
// Matches the original portal's formula
...
function SortIcon({ col, sortKey, dir }: { col: SortKey; sortKey: SortKey; dir: SortDir }) {
  if (col !== sortKey) {
    return <span style={{ color: '#252525', marginLeft: 4, fontSize: 9 }}>↕</span>
  }
  return (
    <span style={{ color: '#c9a96e', marginLeft: 4, fontSize: 9 }}>
      {dir === 'desc' ? '↓' : '↑'}
    </span>
  )
}
...
export function AnalyticsClient({ posts }: { posts: PostRow[] }) {
  ...
  const kpis = useMemo(() => {
    ...
    const avgER       = withViews.length
      ? withViews.reduce((s, r) => s + calcER(r[win]), 0) / withViews.length
      : 0
    const avgWatch    = withViews.length
      ? withViews.reduce((s, r) => s + r[win].watch_pct, 0) / withViews.length
      : 0
    ...
    <div>
      ...
      >
        <KpiCard
          ...
          sub={`${platform.toUpperCase()} · ${WINDOW_LABELS[win]}`}
        />
        <KpiCard
          label="Total Views"
          value={kpis.totalViews > 0 ? fmt(kpis.totalViews) : '—'}
          sub="Sum for selected window"
        />
        <KpiCard
          label="Avg Engagement"
          value={kpis.avgER > 0 ? kpis.avgER.toFixed(1) + '%' : '—'}
          sub="(Likes + cmts + shares + saves) / views"
        />
        <KpiCard
          label="Avg Watch %"
          value={kpis.avgWatch > 0 ? kpis.avgWatch.toFixed(1) + '%' : '—'}
          sub={`${kpis.eliteCount} elite posts (ER ≥12%)`}
        />
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        <span
          className="text-[8px] tracking-[.18em] uppercase self-center mr-1"
          style={{ color: '#252525' }}
        >
          Pillar
        </span>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #141414', background: '#060606' }}>
              ...
              <th className="text-left px-4 py-3 text-[8px] font-medium tracking-[.16em] uppercase"
                  style={{ color: '#2a2a2a', minWidth: 180 }}>
                Title
              </th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[10px] font-medium tracking-[.08em]"
                    ...
                    {/* Views */}
                    <td className="px-4 py-3 text-[12px] font-light text-right" style={{ color: hasData ? '#f2ede4' : '#252525' }}>
                      {hasData ? fmt(w.views) : '—'}
                    </td>

                    {/* Likes */}
                    <td className="px-4 py-3 text-[12px] font-light text-right" style={{ color: hasData ? '#aaa' : '#252525' }}>
                      {hasData ? fmt(w.likes) : '—'}
                    </td>

                    {/* Comments */}
                    <td className="px-4 py-3 text-[12px] font-light text-right" style={{ color: hasData ? '#aaa' : '#252525' }}>
                      {hasData ? fmt(w.comments) : '—'}
                    </td>
      ...
      <div className="flex gap-4 mt-3 flex-wrap">
        ...
        <span className="text-[8px] tracking-[.1em] ml-auto" style={{ color: '#252525' }}>
